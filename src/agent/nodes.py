...
def validate_visitor_node(state: PorteroState) -> PorteroState:
    ...
    if state.cedula:
        ...
        if pre_auth.get("authorized"):
            ...
            # Aquí normalmente llamaríamos al residente
            # Por ahora, mock: asumimos que el residente autorizó
            state.messages.append(AIMessage(
...
def notify_resident_node(state: PorteroState) -> PorteroState:
    ...
    2. Esperar respuesta (por ahora, mock)
    ...
    # Mock: Por ahora, asumimos que el residente siempre autoriza
    # TODO: Integrar con webhook de WhatsApp para capturar respuesta real
    state.resident_contacted = True
    state.resident_authorized = True  # Mock
    state.access_granted = True
    state.authorization_type = AuthorizationType.RESIDENTE
    state.current_step = VisitStep.ACCESO_OTORGADO

    logger.success(f"✅ Residente autorizó el acceso (mock)")

    state.messages.append(AIMessage(
        content=f"El residente ha autorizado su ingreso. Abriendo portón..."
    ))
